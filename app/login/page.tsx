  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4 text-right" dir="rtl">
      <div className="bg-[#0A0A0A] border border-zinc-800 p-8 rounded-[2.5rem] w-full max-w-md shadow-2xl">
        ...
        <p className="text-gold text-[10px] text-center mb-8 uppercase">نظام الإدارة الذكي</p>
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-gold text-black font-black py-4 rounded-2xl shadow-lg">تسجيل الدخول</button>
        </form>
        {status && <div className="mt-4 p-3 bg-zinc-900 rounded-xl text-[10px] text-center text-yellow-200 border border-gold/10">{status}</div>}
      </div>
    </div>
  );
}
