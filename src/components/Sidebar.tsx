...
export default function Sidebar({ userRole, isOpen, onClose }: { userRole: string, isOpen: boolean, onClose: () => void }) {
  ...
  return (
    <>
      {/* طبقة شفافة خلفية عند فتح القائمة في الهاتف */}
      {isOpen && (
      ...
      <div className={`fixed md:static inset-y-0 right-0 w-64 bg-zinc-950 border-l border-zinc-800 p-4 flex flex-col z-50 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'} md:translate-x-0`}>
        <div className="flex justify-between items-center mb-10 md:justify-center pt-4">
          <div className="text-center flex-1">
            <img src="/logo-center.png" className="w-16 h-16 mx-auto mb-2 drop-shadow-[0_0_10px_rgba(212,175,55,0.3)]" />
            <h2 className="text-gold font-black text-xs">نظام معاوية</h2>
          </div>
        ...
        <button 
          ...
          className="mt-auto flex items-center gap-3 p-3 text-red-500 hover:bg-red-500/10 rounded-xl transition"
        >
          <LogOut size={20}/>
